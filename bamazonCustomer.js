...
function askShopInfo() {
  inquirer
    ...
    .then(function(theItemYouPick) {
      ...
      db.query("SELECT * FROM products WHERE item_id=?", yourChoice, function(
        err,
        res
      ) {
        if (err) throw err;
        ...
        if (res.length === 0) {
          console.log(
            "Item does not exist, Please enter a correct id from list"
          );
              ...
              if (numOfItems > res[0].stock_quantity) {
                console.log(
                  "Sorry we only have " +
                    ...
                    " items of this selected product"
                );
                ...
                console.log("--------------");
                console.log(
                  "Total Cost is $" + numOfItems * res[0].price.toFixed(2)
                );
